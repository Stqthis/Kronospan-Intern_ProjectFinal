...
def sibling_choice(question, model, dataframes, max_opts: int = 12):
    ...
    # Labels must be UNIQUE: ten sheets of one workbook all dated
    # 30.06.2024 used to render as ten identical chips. When a date is
    # shared, include the sheet/file name; drop any residual duplicates.
    date_count: dict = {}
    for n in group[:max_opts]:
        d = filename_asof(n) or ""
    ...
    for n in group[:max_opts]:
        ...
        for ext in (".xlsx", ".xls", ".xlsm"):
            if stem.lower().endswith(ext):
                stem = stem[: -len(ext)]
        sheet = n.split(":", 1)[1] if ":" in n else ""
        if d and date_count.get(d, 0) == 1:
            label = d
        elif d:
            label = f"{(sheet or stem)[:34]} ({d})"
        else:
            label = (f"{stem}:{sheet}" if sheet else stem)[:44]
        if label in seen:
            continue
        seen.add(label)
        opts.append({"label": label, "table": n,
                     "suffix": f" as at {d}" if d else f" in file {stem}"})
    opts.sort(key=lambda o: o["label"])
    opts = opts[:8]
    if len(opts) < 2:
        return None, None            # nothing distinguishable to ask about
...
class TableReasoner:
    ...
    def _planner_tables(self, evidence: Evidence) -> dict:
        ...
        if lead:
            chosen = self._ensure_column_coverage(
                evidence.question, lead[:cap], cap)
    ...
    def _ensure_column_coverage(self, question: str, chosen: list,
        ...
        ('Italy' appears in WCR sheets, but only CY05 has Address — the
        ...
        for cs, tables in col_tables.items():
            ...
            try:
                extra = max(tables,
                            key=lambda t: self.dataframes[t].shape[0])
            except Exception:
                extra = tables[0]
            if extra not in out:
                out.append(extra)
