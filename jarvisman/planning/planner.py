...
2. NEVER guess the target table. If multiple tables could plausibly satisfy the request and the schema data does not provide a unique structural mapping, immediately stop and set "clarification": "<short precise question to user>". EXCEPTION: if several tables share the SAME columns and differ only in which rows they hold (e.g. monthly snapshots of one report), the table whose "Distinguishes from sibling sheets" line covers the period or holds the entity the question names IS the correct table -- select it, do not ask.
