...
2. NEVER guess the target table. If multiple tables could plausibly satisfythe request and the schema does not provide a unique mapping, set "clarification". EXCEPTION: if several tables share the SAME columns and differ only by which rows they hold, the table that CONTAINS the value you are filtering on (per the Evidence) IS the correct table — select it.
